export type DateConstructorParam = number | string | Date;

export function secondsSince(date: DateConstructorParam) {
	return Math.floor(msSince(date) / 1000);
}

export function msSince(date: DateConstructorParam) {
	return Date.now() - new Date(date).getTime();
}

export function timeSince(date: DateConstructorParam) {
 ...
 */
export function recencyString(date: DateConstructorParam, breakPoint = 1) {
	date = new Date(date);
 ...
 */
export function dhms(seconds: number) {
 ...
 */
export function dhm(seconds: number) {
